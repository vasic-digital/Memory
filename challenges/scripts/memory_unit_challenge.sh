#!/usr/bin/env bash
# memory_unit_challenge.sh - Validates Memory module unit tests
set -euo pipefail
...
if [ "$pkgs_with_tests" -ge 2 ]; then
    pass "At least 2 packages have tests (found ${pkgs_with_tests})"
else
    fail "Only ${pkgs_with_tests} packages have tests (expected at least 2)"
fi
